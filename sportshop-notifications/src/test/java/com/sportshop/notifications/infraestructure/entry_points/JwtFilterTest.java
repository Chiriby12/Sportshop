...
@DisplayName("JwtFilter (notifications) - Tests del filtro JWT")
class JwtFilterTest {
    ...
    @Test
    @DisplayName("header sin 'Bearer ': continúa sin autenticar")
    ...
    @Test
    @DisplayName("token inválido: no lanza excepción y continúa sin autenticar")
    void tokenInvalido() throws Exception {
        when(request.getHeader("Authorization")).thenReturn("Bearer no.es.jwt");
    ...
    @Test
    @DisplayName("token vacío después de Bearer: no lanza excepción")
